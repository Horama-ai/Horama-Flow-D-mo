...
const eventColors = [
  { main: '#3b82f6', light: '#dbeafe', gradient: 'from-blue-500 to-blue-600' },
  { main: '#8b5cf6', light: '#ede9fe', gradient: 'from-violet-500 to-violet-600' },
  { main: '#06b6d4', light: '#cffafe', gradient: 'from-cyan-500 to-cyan-600' },
  { main: '#f59e0b', light: '#fef3c7', gradient: 'from-amber-500 to-amber-600' },
  { main: '#10b981', light: '#d1fae5', gradient: 'from-emerald-500 to-emerald-600' },
];
        ...
        >
          ...
          <div className={`grid ${getGridCols()} gap-3 lg:gap-4`}>
            {selected.map((event, idx) => {
              ...
              return (
                ...
                >
                  <Card className="p-4 lg:p-5 relative overflow-hidden h-full">
                    <div className={`absolute inset-0 bg-gradient-to-br ${color.gradient} opacity-5`} />
                    <div className="absolute top-0 left-0 right-0 h-1" style={{ backgroundColor: color.main }} />
                    ...
                    <div className="relative">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        >
                          <span className="text-sm font-bold" style={{ color: color.main }}>{score}</span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-gray-50 rounded-lg p-2.5">
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <span className="text-[10px] text-gray-500 uppercase">Fluidité</span>
                          </div>
                          <p className="text-lg font-semibold text-gray-900">{event.globalFluidityScore}%</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-2.5">
                          <div className="flex items-center gap-1.5 mb-1">
                            <Users className="w-3 h-3 text-gray-400" />
                            <span className="text-[10px] text-gray-500 uppercase">Fréquent.</span>
                          </div>
                          <p className="text-lg font-semibold text-gray-900">{(event.currentAttendance / 1000).toFixed(0)}k</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-2.5">
                          <div className="flex items-center gap-1.5 mb-1">
                            <Clock className="w-3 h-3 text-gray-400" />
                            <span className="text-[10px] text-gray-500 uppercase">Attente</span>
                          </div>
                          <p className="text-lg font-semibold text-gray-900">{event.avgWaitTime}min</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-2.5">
                          <div className="flex items-center gap-1.5 mb-1">
                            <TrendingUp className="w-3 h-3 text-gray-400" />
                            <span className="text-[10px] text-gray-500 uppercase">Saturat.</span>
                          </div>
                          <p className="text-lg font-semibold text-gray-900">{event.avgSaturation}%</p>
                        </div>
                      </div>
                    </div>
                  </Card>
          ...
          <div className={`grid grid-cols-1 ${getChartGridCols()} gap-4 lg:gap-6`}>
            ...
            <Card className="p-4 lg:p-6">
              <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Performance globale</h3>
              ...
              <div className="flex flex-wrap justify-center gap-3 mt-4">
                {selected.map((event, idx) => (
                  <div key={event.id} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600">{event.name.substring(0, 15)}</span>
                  </div>
                ))}
              </div>
            </Card>
            ...
            {/* Metrics Bar Chart */}
            <Card className="p-4 lg:p-6">
              <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Métriques clés</h3>
              <div className="h-64 lg:h-72">
                <ResponsiveContainer width="100%" height="100%">
            ...
            {/* Attendance Chart */}
            <Card className="p-4 lg:p-6">
              <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Fréquentation</h3>
              <div className="h-64 lg:h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={attendanceData}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value) => [`${(Number(value) / 1000).toFixed(1)}k visiteurs`]}
                      labelStyle={{ color: 'white', fontWeight: 'bold' }}
                    />
          ...
          <Card className="p-4 lg:p-6 overflow-hidden">
            <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Comparatif détaillé</h3>
            <div className="overflow-x-auto -mx-4 px-4 lg:mx-0 lg:px-0">
              <table className="w-full min-w-[500px]">
                      ...
                      <th key={event.id} className="text-center py-3 px-3">
                        <div className="flex flex-col items-center gap-1">
                          <div
                            ...
                            style={{ backgroundColor: eventColors[idx].light }}
                          >
                            <span className="text-xs font-bold" style={{ color: eventColors[idx].main }}>
                              {idx + 1}
                            </span>
                          </div>
                ...
                <tbody>
                  {[
                    { label: 'Score fluidité', key: 'globalFluidityScore', unit: '%', best: 'max' },
                    { label: 'Fréquentation', key: 'currentAttendance', format: (v: number) => `${(v/1000).toFixed(0)}k`, best: 'max' },
                    { label: 'Attente moyenne', key: 'avgWaitTime', unit: ' min', best: 'min' },
                    { label: 'Densité moyenne', key: 'avgDensity', unit: '%', best: 'min' },
                    { label: 'Saturation', key: 'avgSaturation', unit: '%', best: 'min' },
                    { label: 'Pic fréquentation', key: 'peakAttendance', format: (v: number) => `${(v/1000).toFixed(0)}k`, best: 'max' },
                    { label: 'Nombre de zones', key: 'zones', format: (v: any) => v.length, best: 'none' },
                  ].map((row, rowIdx) => {
                    ...
                    return (
                      <tr key={row.key} className={`border-b border-gray-50 ${rowIdx % 2 === 0 ? 'bg-gray-50/50' : ''}`}>
          ...
          {/* Zone Comparison */}
          <Card className="p-4 lg:p-6">
            <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">Comparaison par zone</h3>
                        ...
                        <div key={event.id} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <span className="text-xs text-gray-600 truncate">{event.name.substring(0, 15)}</span>
                              <div className="flex items-center gap-3 text-xs">
                                <span className="text-gray-500">{z.saturation}%</span>
                                <span className="text-gray-400">{z.waitTime}min</span>
                              </div>
                            </div>
